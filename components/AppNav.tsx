...
export function AppNav({ meta }: AppNavProps) {
  ...
  return (
    ...
    // Frosted linen, sticky. A dark teak bar was tried here and was
    // wrong in context: the rest of the app is an airy cream editorial
    // surface, so a heavy dark ceiling read as a different template
    // pasted on top, pinned the eye to the top of the screen, and pushed
    // the secondary links to a contrast level that was hard to scan.
    // Frosting keeps the useful half — content scrolls under it rather
    // than being cut off — without the weight. Teak stays the ink.
    <header className="nav-glass sticky top-0 z-30 w-full">
      <div className="mx-auto w-full max-w-2xl px-6 py-4 sm:py-5">
      <div className="flex items-center justify-between gap-4">
        <Link href="/browse" className="display text-xl">
      ...
      {/* Scrolls sideways rather than wrapping if it ever overflows —
          ...
          not as a scroller. */}
      <div className="label mt-2.5 flex items-center gap-x-5 overflow-x-auto whitespace-nowrap text-[var(--muted)] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {meta && <span className="shrink-0 text-[var(--terracotta)]">{meta}</span>}
